...
Fast mode has separate desired/effective state. If you enable fast mode while using an unsupported model, the extension remembers that fast mode is desired, warns that the current model is unsupported, and leaves effective fast mode off until you switch to a supported model.
...
- `persistState`: persist fast-mode state across sessions.
...
- `image.enabled`: allow the `openai_image` tool to make image requests.
...
The `openai_image` tool calls `https://chatgpt.com/backend-api/codex/responses` with the hosted Responses `image_generation` tool and the `openai-codex` OAuth credentials from `/login openai-codex`. This follows Codex CLI's subscription-auth path and does not require `OPENAI_API_KEY`.
...
- `extensions/image.ts` owns OpenAI Codex subscription-auth image generation and image saving.
